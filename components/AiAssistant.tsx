...
export default function AiAssistant() {
  ...
  if (!isOpen) {
    return (
      <div
        ...
        className="fixed bottom-6 right-6 bg-blue-600 text-white px-6 py-4 rounded-full shadow-[0_10px_40px_rgba(37,99,235,0.4)] z-[9999] font-bold cursor-pointer transition-all hover:scale-110 active:scale-95 flex items-center gap-3 border border-blue-400"
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[380px] h-[550px] bg-white border border-blue-100 shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-[2rem] flex flex-col z-[9999] overflow-hidden text-slate-800 animate-in fade-in slide-in-from-bottom-8 duration-300">
      ...
      {/* Шапка чата */}
      <div className="bg-gradient-to-r from-blue-500 to-blue-700 p-5 text-white flex justify-between items-center shadow-lg border-b border-blue-100">
        <div className="flex items-center gap-3">
          <div className="bg-white/20 p-2.5 rounded-xl backdrop-blur-md">
            <Bot className="w-6 h-6 text-white" />
          </div>
          <div>
            <h3 className="font-extrabold text-sm tracking-wide leading-tight">KomekArch AI</h3>
            <div className="text-[10px] text-blue-100 mt-1 flex items-center gap-1.5 font-medium">
              <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span>
              Онлайн | Поддержка CS-201
            </div>
          </div>
        </div>
        <button
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
        ...
        {messages.map((m, idx) => (
          // ИСПРАВЛЕНО: Добавлены обратные кавычки для динамических классов
          <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
            <div className={`max-w-[85%] p-4 rounded-2xl text-[13px] leading-relaxed shadow-sm ${m.role === 'user'
        ...
        {isLoading && (
          <div className="flex justify-start animate-pulse p-2">
            <div className="bg-slate-200 h-8 w-16 rounded-full flex items-center justify-center gap-1">
