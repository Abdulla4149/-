...
import { LockKeyhole, Mail, User, UserPlus, AlertTriangle } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    ...
    try {
      ...
      if (!res.ok) {
        setServerError(json?.error || "Не удалось зарегистрироваться");
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-100 p-4">
      <div className="w-full max-w-md bg-white rounded-[2rem] shadow-2xl shadow-blue-200/50 overflow-hidden border border-white/20 animate-in fade-in zoom-in duration-500">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-8 text-center">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight">Создать аккаунт</h1>
          <p className="text-blue-100 text-sm mt-2 font-medium">Регистрация в KomekArch</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none transition-all text-sm focus:ring-4 focus:bg-white focus:ring-blue-100 focus:border-blue-400"
              />
            </div>
          </div>
          ...
          >
            <span>{isLoading ? "Создаем..." : "Создать аккаунт"}</span>
